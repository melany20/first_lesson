...
- [This is only an example. Replace the URI with your own repository address.](#this-is-only-an-example-replace-the-uri-with-your-own-repository-address)
- [Разрешение конфликтов и другие полезные штуки](#разрешение-конфликтов-и-другие-полезные-штуки)

# Что такое Git?

Git — это набор консольных утилит, которые отслеживают и фиксируют изменения в файлах. Git создал Линус Торвальдс при разработке ядра Linux: инструмент так понравился разработчикам, что его стали использовать в других проектах. Git позволяет сравнивать, анализировать, редактировать, сливать изменения и возвращаться назад к последнему сохранению. Этот процесс называется контролем версий. 

## Преимущества Git

* Мы можем посмотреть как менялись файлы программы, а если нужно, вернуться назад и что-то отредактировать. Случается, что после добавления новых функций код ломается, и нужно срочно откатиться к предыдущей, отлично работающей версии. Без системы контроля версий пришлось бы шерстить код, чтобы понять как было до того, как все перестало работать, а с Git нужно просто откатиться на коммит назад.

* Возможность одновременной работы над проектом нескольких специалистов. Даже если вся команда решит залить изменения в своих файлах в один и тот же момент, проект не сломется. 

* Git не зависит от одного центрального сервера, на котором хранятся файлы. Он работает полностью локально, сохраняет данные в директории на жестком диске, или репозитории. При этом хранить копию репозитория можно хранить онлайн, чтобы над одним проектом могли работать сразу нескольких людей. В этом помогают GitHub, BitBucket и другие сервисы.

# Установка и настройка

Алгоритм установки Git зависит от ОС:
...
Чтобы изменения, которые вы внесли, были подписаны вашим именем и имейлом, запустите в терминале команды:
'''
git config --global user.name "My Name"
git config --global user.email myEmail@example.com
'''

Git хранит весь пакет конфигураций в файле .gitconfig в вашем локальном каталоге. Чтобы сделать эти настройки глобальными, то есть применимыми ко всем проектам, необходимо добавить флаг –global. Если вы этого не сделаете, они будут распространяться только на текущий репозиторий.
...
# Разрешение конфликтов и другие полезные штуки
